...
def iterate_train_data(args, train_hot_ld, train_normal_ld, hot_emb_dict, val_ld, tbsm, k, use_gpu, device, writer, losses, accuracies, isMainTraining, hot_entry_samples, ln_hot_emb, train_hot):
        ...
        for j, (X, lS_o, lS_i, T) in enumerate(train_normal_ld):
                ...
                if isMainTraining:
                        should_test = (
                                (args.test_freq > 0
                                and (j + 1) % args.test_freq == 0) or (j + 1 == nbatches_hot) or (j == 0)
                        )
                else:
                ...
                if should_test:
                        ...
                        '''
                        print("Test_Iteration ", nbatches_hot + j)
                        print("Total_Iterations ", nbatches)
        ...
        for j, (X, lS_o, lS_i, T) in enumerate(train_hot_ld):
                ...
                backward_sampling_time += end_optimizing - end_backward
                ...
                print_tl = (j == 0) or ((j + 1) % args.print_freq == 0) or (j + 1 == nbatches_hot)
                # print time, loss and accuracy
                if print_tl and isMainTraining:
        ...
        hot_overall_training = hot_forward_time + hot_backward_time+ hot_optimizer_time + hot_scheduler_time
        ...
        print("=====================================================================")
        print("Overall_Training_Time_Sec :",cold_overall_training + hot_overall_training + overall_emb_update)
        print("Overall_Training_Testing_Time_Sec :",cold_overall_training + hot_overall_training + test_time_hot + test_time_cold + overall_emb_update )
        print("=====================================================================")
        print(" ")
...
def worker_func(train_hot,non_changing_index,final_total, cluster_0, cluster_1, chunksize):
    ...
    for i, hot_tuple in enumerate(train_hot):
        lS_i_temp = []
        #print(" hot_tuple ",hot_tuple)
        #print(" hot_tuple[0][1] ", hot_tuple[0][1])

        #print("len(tuple[0] : ",len(hot_tuple[0]))
        #exit()
        ...
        for j, lS_i_row in enumerate(hot_tuple[0]):
            ...
            for k, lS_i_index in enumerate(lS_i_row):
                #print(" lS_i : ",lS_i_index)
                #exit()
                if final_total[int(lS_i_index)] == 1:
                    lS_i_temp.append(int(lS_i_index))
                else:
                    break
