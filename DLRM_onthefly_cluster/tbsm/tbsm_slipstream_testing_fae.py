...
def iterate_train_data(args, train_hot_ld, train_normal_ld, hot_emb_dict, val_ld, tbsm, k, use_gpu, device, writer, losses, accuracies, isMainTraining, hot_entry_samples, ln_hot_emb, train_hot):
		...
		for j, (X, lS_o, lS_i, T) in enumerate(train_hot_ld):
				...
				forward_time = end_forward_sampling - begin_forward_sampling
				backward_time = end_backward_sampling - end_forward_sampling
				optimizer_time = end_optimizing_sampling - end_backward_sampling
		...
		print(" slipstream_overhead_without_sampling_hot_embedding_table : ",total_threshold_setting_time)
		#exit()
