...
class TSL_Net(nn.Module):
	...
	def forward(self, x=None, H=None):
		...
		if self.model_type == "tsl":
			...
			max_sequence_length, d_model = , 64
...
def train_tbsm(args, use_gpu):
	...
	lstr = args.dlrm_path.split("/")
	d_path = "/".join(lstr[0:-1]) + "/"
	d_file = lstr[-1]
	train_ld, _ = tp.make_tbsm_data_and_loader(args, "train",d_file,d_path)
...
if __name__ == "__main__":
	...
	# model_type = "tsl", "mha", "rnn" , "transformers"
	#parser.add_argument("--model-type", type=str, default="tsl")  # tsl, mha, rnn , transformers
	parser.add_argument("--model-type", type=str, default="transformers")  # tsl, mha, rnn , transformers
